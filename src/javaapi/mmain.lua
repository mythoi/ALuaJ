...
if app_theme.colorPrimary=="#222222" then
  methodDlg=Dialog(activity,R.style.AppTheme2)
else
  methodDlg=Dialog(activity,R.style.AppTheme)
end

methodDlg.getWindow().setStatusBarColor(Color.parseColor(app_theme.colorPrimary));

methodDlg.setContentView(loadlayout(mlayout))
curr_class=nil
curt_adapter=nil

showMethodDlg=function(text)
